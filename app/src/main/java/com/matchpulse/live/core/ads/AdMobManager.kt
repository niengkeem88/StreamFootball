...
class AdMobManager @Inject constructor(
    ...
    fun gatherConsentAndInitialize(activity: Activity) {
        consentManager.gatherConsent(activity) { error ->
            if (error == null) {
                ...
                Log.e(ADS_LOG_TAG, lastEvent)
                // Initialize anyway if we failed to gather consent but had a previous state?
                // Actually, AdMob recommendation is to try initialize if canRequestAds is true.
                if (consentManager.canRequestAds) {
                    initialize(activity.applicationContext)
                }
            }
    ...
    fun initialize(context: Context) {
        ...
        runCatching {
            ...
            Log.w(ADS_LOG_TAG, lastEvent, throwable)
        }
    }
    ...
    fun showPrivacyOptions(activity: Activity) {
        consentManager.showPrivacyOptionsForm(activity) { error ->
            if (error != null) {
                Log.e(ADS_LOG_TAG, "Privacy options form error: ${error.message}")
            }
        }
    }
